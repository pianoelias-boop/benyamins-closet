...
1. **Read his signals.** Pull the notebook sheet (`latest` tab, or `GET <syncUrl>?key=<notebookKey>`, both from `config.js`) and
   update `profile.md` with what he saves and passes: brands, silhouettes, colours, fabrics, price band.
   While the notebook is empty the closet itself (`data.js`) stands in for his taste.
...
6. **Fold.** Ideas he has hearted (kind `s` on a 5000+ id in the notebook) can be moved into
   `build/extras.json` permanently; ideas he passed stay out and count against the brand next round.
...
his profile, and labels carried by stores he already hearts. Vet before proposing: real product pages,
menswear, fibre content published, prices overlapping the range, a physical home and returns policy.
